...
AI::AudioTimbreAnalyzer::AnalysisResult PluginProcessor::analyzeTimbreFromBuffer(
    ...
    // 优先使用ONNX模型，否则使用回退模式
    if (timbreAnalyzer_.isLoaded()) {
        lastTimbreResult_ = timbreAnalyzer_.analyze(buffer, sampleRate);
    } else {
        lastTimbreResult_ = timbreAnalyzer_.analyzeFallback(buffer, sampleRate);
    }
    
    analyzing_ = false;
    return lastTimbreResult_;
}
